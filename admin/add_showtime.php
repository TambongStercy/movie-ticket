...
$movie_id = $theatre_id = $show_datetime = $price_per_seat = '';
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || !validateCsrfToken($_POST['csrf_token'])) {
        ...
        if (empty($errors)) {
            $stmt = $pdo->prepare('INSERT INTO showtimes (movie_id, theatre_id, show_datetime, price_per_seat) VALUES (?, ?, ?, ?)');
            $stmt->execute([$movie_id, $theatre_id, $show_datetime, $price_per_seat]);
...
<div class="flex">
    <div class="ml-[240px] pt-20 w-full min-h-screen p-8 bg-[#18181c]">
        <div class="max-w-lg mx-auto card-magic">
            <h1 class="text-2xl font-bold text-white mb-6">Add New Showtime</h1>
            <?php if ($errors): ?>
            ...
            <form method="POST" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-200 mb-1">Date & Time <span
                            class="text-red-500">*</span></label>
                    <input type="datetime-local" name="show_datetime" value="<?php echo escape($show_datetime); ?>"
                        class="w-full px-3 py-2 bg-[#23232b] border border-[#333] rounded text-white focus:ring-red-500 focus:border-red-500">
                </div>
                <div>
                ...
                <div class="flex flex-col md:flex-row gap-2 pt-2">
                    <button type="submit" class="w-full btn-magic">Add Showtime</button>
